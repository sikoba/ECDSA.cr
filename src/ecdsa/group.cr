module ECDSA
  class Group
    ...
    def sign(secret_key : BigInt, message : String, force_lower_s : Bool = true) : Signature
      ...
      sign(secret_key, e, temp_key_k, force_lower_s)
    end

    def sign_sha3_256(secret_key : BigInt, message : String, force_lower_s : Bool = true) : Signature
    ...
    def sign_keccak_256(secret_key : BigInt, message : String, force_lower_s : Bool = true) : Signature
    ...
    def sign(secret_key : BigInt, message : String, temp_key_k : BigInt, force_lower_s : Bool = true) : Signature
      hash = Digest::SHA256.hexdigest(message)
    ...
    def sign(secret_key : BigInt, e : BigInt, force_lower_s : Bool = true) : Signature
      temp_key_k = ECDSA::Math.random(BigInt.new(1), n - 1)
      sign(secret_key, e, temp_key_k, force_lower_s)
    end

    def sign(secret_key : BigInt, e : BigInt, temp_key_k : BigInt, force_lower_s : Bool = true) : Signature
      ...
      # compute v ()
      even_y = curve_point.y % 2 == 0 ? true : false
      ...
      Signature.new(r, s, even_y, lowered_s)
    end
